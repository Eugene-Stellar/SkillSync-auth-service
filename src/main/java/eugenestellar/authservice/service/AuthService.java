...
public class AuthService {
  ...
  public ResponseTokenDto register(AuthRegisterUserDto userDto) {
    ...
    String token = jwtUtil.generateToken(savedUser.getId(), email, true, List.of(userForDb.getRole().toString()));
    return new ResponseTokenDto(token, setRefreshToken(email));
  }

  public String setRefreshToken(String email) {
    return jwtUtil.generateToken(null, email, false, null);
  }
  ...
  public ResponseTokenDto login(AuthLoginUserDto userDto) {
    ...
    String role = userFromDb.getRole().toString();
    String token = jwtUtil.generateToken(userFromDb.getId(), userFromDb.getEmail(), true, List.of(role));

    return new ResponseTokenDto(token, jwtUtil.generateToken(null, userFromDb.getEmail(), false, null));
  }
  ...
  public String getNewAccessToken(String refreshToken) {
    try {
      ...
      User user = userRepo.findByEmail(email)
          .orElseThrow(() -> new UserNotFoundForTokenException("User not found with email: " + email));

      String role = user.getRole().toString();

      return jwtUtil.generateToken(user.getId(), email, true, List.of(role));
